...
public class MagicBakery implements Serializable {
     ...
     * 
     * @param seed               The seed to initialise the Random object, for
    ...
    public void bakeLayer(Layer layer) {
        ...
        if (getBakeableLayers().contains(layer)) {
            ...
            throw new WrongIngredientsException(
                    "Ingredients or substitutable number of helpful ducks required to bake not found!");
        }
    }
    ...
    private Ingredient drawFromPantryDeck() {
        if (this.pantryDeck.isEmpty()) {
            if (this.pantryDiscard.isEmpty()) {
                throw new EmptyPantryException("The pantry is empty, use the cards in your hand to bake.",
    ...
    /**
     * Draws an ingredient card with name ingredientName from the pantry and places
     * it in the current player's hand.
     *
    ...
    public void drawFromPantry(String ingredientName) {
        ...
        if (this.pantry.contains(new Ingredient(ingredientName))) {
            ((ArrayList<Ingredient>) this.pantry).set(
                    ((ArrayList<Ingredient>) this.pantry).indexOf(new Ingredient(ingredientName)),
                    drawFromPantryDeck());
    ...
    /**
     * Draws a card, ingredient, from the pantry and places it in the current
     * player's hand.
     * 
    ...
    public void drawFromPantry(Ingredient ingredient) {
        ...
        if (this.pantry.contains(ingredient)) {
            ((ArrayList<Ingredient>) this.pantry).set(((ArrayList<Ingredient>) this.pantry).indexOf(ingredient),
                    drawFromPantryDeck());
    ...
    /**
     * Changes turns among players if it is the end of the turn for the current
     * player.
     * 
    ...
    /**
     * Fulfills the customer order if fulfillable, and garnishes the order if
     * garnish is true and if garnishable.
     * 
     * @param customer Customer Order to fulfill(and/or garnish)
     * @param garnish  Whether the order should be garnished
     * @return List of ingredients excluding the ones used to fulfill(and/or
     *         garnish) the order.
     */
    ...
    public Collection<CustomerOrder> getGarnishableCustomers() {
        ...
        for (CustomerOrder customer : customers.getActiveCustomers()) {
            if (customer.canGarnish(this.currentPlayer.getHand())
                    && customer.canFulfill(this.currentPlayer.getHand())) {
     ...
     * @throws IOException            If there are any problems reading the file.
     * @throws ClassNotFoundException If the object loaded does not belong to any
     *                                class.
     */
    ...
    /**
     * Passes the specified ingredient card from the current player's hand to the
     * recipient's.
     * 
    ...
    public void passCard(Ingredient ingredient, Player recipient) {
        ...
        if (!this.currentPlayer.getHand().contains(ingredient)) {
            throw new WrongIngredientsException(
                    "Ingredient card not found in current player's hand; Cannot pass to another player");
        }
        this.currentPlayer.removeFromHand(ingredient);
    ...
    /**
     * Prints the customer service record, which includes the number of customers
     * fulfilled(and garnished)
     * and the disappointed customers who left.
     */
    public void printCustomerServiceRecord() {
        int fulfilled = this.customers.getInactiveCustomersWithStatus(CustomerOrder.CustomerOrderStatus.FULFILLED)
                .size();
        int garnished = this.customers.getInactiveCustomersWithStatus(CustomerOrder.CustomerOrderStatus.GARNISHED)
                .size();
        int abandoned = this.customers.getInactiveCustomersWithStatus(CustomerOrder.CustomerOrderStatus.GIVEN_UP)
                .size();

        System.out.println(
                "Happy customers eating baked goods: " + (fulfilled + garnished) + " (" + garnished + " garnished) \n" +
                        "Gone to Greggs instead: " + abandoned);
    }

    /**
     * Prints the current state of the game, including the layers available to bake,
     * the pantry, the customer orders yet be fulfilled and the current player's
     * hand.
     */
    ...
    /**
     * Refreshes the pantry by discarding the current contents and drawing new items
     * from the pantry deck.
